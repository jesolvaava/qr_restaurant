...
def payment(request):
    if request.method == 'POST':
        try:
            ...
            # Validate 10-digit UPI ID
            if not upi_id.isdigit() or len(upi_id) != 10:
                return JsonResponse({'status': 'error', 'message': 'Invalid UPI ID. Please enter a 10-digit number.'}, status=400)
